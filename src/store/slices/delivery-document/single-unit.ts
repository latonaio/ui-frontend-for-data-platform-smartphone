...
import {
  DeliveryDocumentSingleUnitProps as DefaultDeliveryDocumentSingleUnitProps,
...
export interface DeliveryDocumentSingleUnitProps extends DefaultDeliveryDocumentSingleUnitProps {
  isEditing: {
    [key: string]: boolean;
  };
  errors: errors
...
export interface InitialState {
  [DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit]: DeliveryDocumentSingleUnitProps | null;
}

const initialState: InitialState = {
  [DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit]: null,
};
...
interface editItemParam {
  params: {
    product: {
      product: number;
      Item: DeliveryDocumentSingleUnitProps;
    };
    accepter: string[];
    api_type: string;
  };
...
export const deliveryDocumentSingleUnit = createSlice({
  name: 'deliveryDocumentSingleUnit',
  initialState,
  reducers: {
    initializeUpdate: (state, action: PayloadAction<{
      [DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit]: DefaultDeliveryDocumentSingleUnitProps;
    }>) => {
      const errors = (): errors => {
      ...
      if (!action.payload[DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit]) { return; }

      state[DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit] = {
        isEditing: isEditingObject,
        errors: errors(),
        ...action.payload[DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit],
      }
    },
    pushItemToEdit: (state: InitialState, action: PayloadAction<pushEditItem>) => {
      const targetState = state[DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit];
...
const isError = (index: number, detailState: {
  ...
  const detail  = detailState[DeliveryDocumentTablesEnum.deliveryDocumentSingleUnit];
...
export const editItemAsync = async (
  ...
  try {
    ...
    appDispatch(deliveryDocumentSingleUnit.actions.editedItem(
      {
        index: index,
...
export const {
  ...
  closeItem,
  setErrorItem,
} = deliveryDocumentSingleUnit.actions;
