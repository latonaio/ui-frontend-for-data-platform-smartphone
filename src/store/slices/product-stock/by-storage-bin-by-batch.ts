...
import {
  ProductStockByStorageBinByBatchProps as DefaultProductStockByStorageBinByBatchProps,
  ProductStockTablesEnum,
} from '@/constants';
import { setLoading } from '@/store/slices/loadging';
// import { updates } from '@/api/productStock/ByStorageBinByBatch';
...
export interface ProductStockByStorageBinByBatchProps extends DefaultProductStockByStorageBinByBatchProps {
  isEditing: {
    [key: string]: boolean;
  };
  errors: errors
...
export interface InitialState {
  [ProductStockTablesEnum.productStockByStorageBinByBatch]: ProductStockByStorageBinByBatchProps | null;
}

const initialState: InitialState = {
  [ProductStockTablesEnum.productStockByStorageBinByBatch]: null,
};
...
interface editItemParam {
  params: {
    ProductStock: {
      ProductStock: number;
      Item: ProductStockByStorageBinByBatchProps;
    };
    accepter: string[];
    api_type: string;
  };
...
export const productStockByStorageBinByBatch = createSlice({
  name: 'productStockByStorageBinByBatch',
  initialState,
  reducers: {
    initializeUpdate: (state, action: PayloadAction<{
      [ProductStockTablesEnum.productStockByStorageBinByBatch]: DefaultProductStockByStorageBinByBatchProps;
    }>) => {
      const errors = (): errors => {
      ...
      if (!action.payload[ProductStockTablesEnum.productStockByStorageBinByBatch]) { return; }

      state[ProductStockTablesEnum.productStockByStorageBinByBatch] = {
        isEditing: isEditingObject,
        errors: errors(),
        ...action.payload[ProductStockTablesEnum.productStockByStorageBinByBatch],
      }
    },
    pushItemToEdit: (state: InitialState, action: PayloadAction<pushEditItem>) => {
      const targetState = state[ProductStockTablesEnum.productStockByStorageBinByBatch];
...
const isError = (index: number, detailState: {
  ...
  const detail  = detailState[ProductStockTablesEnum.productStockByStorageBinByBatch];
...
export const editItemAsync = async (
  ...
  try {
    ...
    appDispatch(productStockByStorageBinByBatch.actions.editedItem(
      {
        index: index,
...
export const {
  ...
  closeItem,
  setErrorItem,
} = productStockByStorageBinByBatch.actions;
