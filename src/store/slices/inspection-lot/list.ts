...
import {
  InspectionLotListProps as DefaultInspectionLotListProps,
...
export interface InspectionLotListProps extends DefaultInspectionLotListProps {
  isEditing: {
    [key: string]: boolean;
  };
  errors: errors
...
export interface InitialState {
  [InspectionLotTablesEnum.inspectionLotList]: InspectionLotListProps | null;
}

const initialState: InitialState = {
  [InspectionLotTablesEnum.inspectionLotList]: null,
};
...
interface pushEditItem {
  index: number;
  item: InspectionLotListProps;
  key: string;
}

export const inspectionLotList = createSlice({
  name: 'inspectionLotList',
  initialState,
  reducers: {
    initializeUpdate: (state, action: PayloadAction<{
      [InspectionLotTablesEnum.inspectionLotList]: DefaultInspectionLotListProps;
    }>) => {
      const errors = (): errors => {
      ...
      if (!action.payload[InspectionLotTablesEnum.inspectionLotList]) { return; }

      state[InspectionLotTablesEnum.inspectionLotList] = {
        isEditing: isEditingObject,
        errors: errors(),
        ...action.payload[InspectionLotTablesEnum.inspectionLotList],
      }

      if (!state[InspectionLotTablesEnum.inspectionLotList]) { return; }
      if (!action.payload[InspectionLotTablesEnum.inspectionLotList].Partner) { return; }
      if (action.payload[InspectionLotTablesEnum.inspectionLotList].Partner.length == 0) { return; }

      // item に対して isEditing と errors を追加
      state[InspectionLotTablesEnum.inspectionLotList].Partner = action.payload[InspectionLotTablesEnum.inspectionLotList].Partner
        .map((item) => {
          return {
            ...item,
            isEditing: isEditingObject,
    ...
    getState: (state: any) => {
      return state[InspectionLotTablesEnum.inspectionLotList];
    },
    pushItemToEdit: (state: InitialState, action: PayloadAction<pushEditItem>) => {
      const targetState = state[InspectionLotTablesEnum.inspectionLotList];
      ...
      state[InspectionLotTablesEnum.inspectionLotList].Partner[actionData.index] = {
        ...actionData.item,
      }
    },
    editedItem: (state, action: PayloadAction<{
      ...
      const targetState = actionData.listState[InspectionLotTablesEnum.inspectionLotList]
      ...
      state[InspectionLotTablesEnum.inspectionLotList].Partner[actionData.updateInfoObject.index] = {
        ...targetState,
      }
    },
...
const isError = (index: number, detailState: {
  ...
  const list  = detailState[InspectionLotTablesEnum.inspectionLotList].Item;
...
export const editItemAsync = async (
  ...
  try {
    ...
    appDispatch(inspectionLotList.actions.editedItem(
      {
        listState: listState,
...
export const {
  ...
  closeItem,
  setErrorItem,
} = inspectionLotList.actions;

export default inspectionLotList.reducer
